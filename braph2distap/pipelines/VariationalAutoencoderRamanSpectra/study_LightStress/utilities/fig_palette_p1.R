# ----- I/O configuration for figures -----
# If an output directory is passed as a command-line argument,
# use that. Otherwise, default to the current working directory.
...
for (i in 1:3){
  ...
  if (i == 1) {
    ggsave(file.path(out_dir, "AB_1d_ll.jpeg"),
       units = "in", width = w, height = h, device = "tiff", dpi = 600)
  }
  
  if (i == 2) {
    ggsave(file.path(out_dir, "CS_1d_ll.jpeg"),
       units = "in", width = w, height = h, device = "tiff", dpi = 600)
  }
  
  if (i == 3) {
    ggsave(file.path(out_dir, "KL_1d_ll.jpeg"),
       units = "in", width = w, height = h, device = "tiff", dpi = 600)
  }
}

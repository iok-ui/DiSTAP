...
for (i in 1:length(files)){
  ...
  if (grepl('AB', filename)){
    ggsave(path=save_filepath, filename="lsqmed_AB.jpeg", 
           units="in", width = w, height = h, device='tiff', dpi=600)
    ggsave(path=save_filepath, filename="lsqmed_AB_w_legend.jpeg", 
           units="in", width = 5, height = 4, device='tiff', dpi=600)
  }
  
  if (grepl('CS', filename)){
    ggsave(path=save_filepath, filename="lsqmed_CS.jpeg", 
           units="in", width = w, height = h, device='tiff', dpi=600)
  }
  
  if (grepl('KL', filename)){
    ggsave(path=save_filepath, filename="lsqmed_KL.jpeg", 
           units="in", width = w, height = h, device='tiff', dpi=600)
  }
}
setwd(save_filepath)

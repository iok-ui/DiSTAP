...
list.of.packages <- c("ggpubr", "R.matlab", "hdf5r", "tidyr", "tidyverse", "cowplot", "hrbrthemes", "ggplot2", "ggpattern", "magick", "here", "dplyr", "grid")
...
for (i in 1:length(files)) {
  ...
  # Assign shades
  z1_df_1$shade <- 'Deep shade'
  z1_df_2$shade <- 'Moderate shade'
  z1_df_3$shade <- 'White light'
  ...
  # Assign shades
  z2_df_1$shade <- 'Deep shade'
  z2_df_2$shade <- 'Moderate shade'
  z2_df_3$shade <- 'White light'
  ...
  # Plot using ggscatter with custom tick positions and labels
    ls <- ggscatter(
      ...
      ellipse = FALSE,
      #mean.point = TRUE,
      #mean.point.size = 4,         # Make mean points larger
      #mean.point.fill = NULL,      # Match the fill color to cluster color
      star.plot = FALSE,
      alpha = 0.3,
      ggtheme = theme_bw()
    ) +
    ...
    labs(
      x = "z1",
      y = "z2",
      #title = "Scatter plot of erf(z1) vs erf(z2) with inverse erf tick labels"
    ) +
    expand_limits(x = c(-1, 1), y = c(-1, 1)) +  
    scale_x_continuous(
      breaks = tick_positions,    # Positions on erf scale
      labels = tick_labels        # Display original inverse erf values
    ) +
    scale_y_continuous(
    ...
    theme(
      ...
      plot.margin = margin(0, 0, 0, -20)  # Adjust margins (top, right, bottom, left)
    ) +
      coord_fixed(ratio = 1) + # Maintain aspect ratio while reducing white space
    # Add median points manually
    geom_point(
      data = median_points,
  ...
  w = 2.75
  h = 2.75
  save_filepath = paste(read_filepath, '../', sep="")

  if (grepl('PhyA_and_leaf', filename)) {
    ggsave(path = save_filepath, filename = "lsqmed_PhyA_leaf.jpeg",
           units = "in", width = w, height = h, device = 'tiff', dpi = 600)
    ggsave(path = save_filepath, filename = "lsqmed_PhyA_leaf_w_legend.jpeg",
           units = "in", width = 4, height = 4, device = 'tiff', dpi = 600)
  }

  if (grepl('PhyB_and_leaf', filename)) {
    ggsave(path = save_filepath, filename = "lsqmed_PhyB_leaf.jpeg",
           units = "in", width = w, height = h, device = 'tiff', dpi = 600)
  }

  if (grepl('WT_and_leaf', filename)) {
    ggsave(path = save_filepath, filename = "lsqmed_WT_leaf.jpeg",
           units = "in", width = w, height = h, device = 'tiff', dpi = 600)
  }

  if (grepl('PhyA_and_petiole', filename)) {
    ggsave(path = save_filepath, filename = "lsqmed_PhyA_petiole.jpeg",
           units = "in", width = w, height = h, device = 'tiff', dpi = 600)
  }

  if (grepl('PhyB_and_petiole', filename)) {
    ggsave(path = save_filepath, filename = "lsqmed_PhyB_petiole.jpeg",
           units = "in", width = w, height = h, device = 'tiff', dpi = 600)
  }

  if (grepl('WT_and_petiole', filename)) {
    ggsave(path = save_filepath, filename = "lsqmed_WT_petiole.jpeg",
           units = "in", width = w, height = h, device = 'tiff', dpi = 600)
  }
}
setwd(save_filepath)

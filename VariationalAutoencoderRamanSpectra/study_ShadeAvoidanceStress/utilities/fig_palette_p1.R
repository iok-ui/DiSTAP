...
for (i in 1:6){
  ...
  if (i==1) {
    filename1 <- "(Tr)_latent_space_ds_and_ms_and_wl_at_BnyWT_and_leaf.mat"
    filename2 <- "(Tr) Diff Spectrum (ds-ms-wl) with BnyWT and leaf.mat"
    filename3a <- "ranked_sig_pks_wl BnyWT and leaf.mat"
    filename3b <- "ranked_sig_pks_ms BnyWT and leaf.mat"
    filename3c <- "ranked_sig_pks_ds BnyWT and leaf.mat"
    group_id <- 'WT'
  }
  
  if (i==2) {
    filename1 <- "(Tr)_latent_space_ds_and_ms_and_wl_at_BnyPhyA_and_leaf.mat"
    filename2 <- "(Tr) Diff Spectrum (ds-ms-wl) with BnyPhyA and leaf.mat"
    filename3a <- "ranked_sig_pks_wl BnyPhyA and leaf.mat"
    filename3b <- "ranked_sig_pks_ms BnyPhyA and leaf.mat"
    filename3c <- "ranked_sig_pks_ds BnyPhyA and leaf.mat"
    group_id <- 'PhyA'
  }
  
  if (i==3) {
    filename1 <- "(Tr)_latent_space_ds_and_ms_and_wl_at_BnyPhyB_and_leaf.mat"
    filename2 <- "(Tr) Diff Spectrum (ds-ms-wl) with BnyPhyB and leaf.mat"
    filename3a <- "ranked_sig_pks_wl BnyPhyB and leaf.mat"
    filename3b <- "ranked_sig_pks_ms BnyPhyB and leaf.mat"
    filename3c <- "ranked_sig_pks_ds BnyPhyB and leaf.mat"
    group_id <- 'PhyB'
  }
  
  if (i==4) {
    filename1 <- "(Tr)_latent_space_ds_and_ms_and_wl_at_BnyWT_and_petiole.mat"
    filename2 <- "(Tr) Diff Spectrum (ds-ms-wl) with BnyWT and petiole.mat"
    filename3a <- "ranked_sig_pks_wl BnyWT and petiole.mat"
    filename3b <- "ranked_sig_pks_ms BnyWT and petiole.mat"
    filename3c <- "ranked_sig_pks_ds BnyWT and petiole.mat"
    group_id <- 'WT'
  }
  
  if (i==5) {
    filename1 <- "(Tr)_latent_space_ds_and_ms_and_wl_at_BnyPhyA_and_petiole.mat"
    filename2 <- "(Tr) Diff Spectrum (ds-ms-wl) with BnyPhyA and petiole.mat"
    filename3a <- "ranked_sig_pks_wl BnyPhyA and petiole.mat"
    filename3b <- "ranked_sig_pks_ms BnyPhyA and petiole.mat"
    filename3c <- "ranked_sig_pks_ds BnyPhyA and petiole.mat"
    group_id <- 'PhyA'
  }
  
    if (i==6) {
    filename1 <- "(Tr)_latent_space_ds_and_ms_and_wl_at_BnyPhyB_and_petiole.mat"
    filename2 <- "(Tr) Diff Spectrum (ds-ms-wl) with BnyPhyB and petiole.mat"
    filename3a <- "ranked_sig_pks_wl BnyPhyB and petiole.mat"
    filename3b <- "ranked_sig_pks_ms BnyPhyB and petiole.mat"
    filename3c <- "ranked_sig_pks_ds BnyPhyB and petiole.mat"
    group_id <- 'PhyB'
  }
  ...
  # z1_df_3$shade <- 'WL'
  # 
  # z1_df <- rbind(z1_df_1, z1_df_2, z1_df_3)
  # 
  # z2 <- data$z2
  ...
  # z2_df_3$shade <- 'WL'
  # 
  # z2_df <- rbind(z2_df_1, z2_df_2, z2_df_3)
  # 
  ...
  #           color = "shade", palette = c("#DC0000FF", "#4DBBD5FF", "#7E6148FF"),
  #           shape = "shade",
  #           ellipse = FALSE, 
  #           mean.point = TRUE,
  #           star.plot = FALSE, 
  ...
  #                   pattern_fill2 = "#DC000099") +
    ...
    theme(plot.title = element_text(size = 12),
          ...
          axis.line.x        = element_blank(), #element_blank()
          axis.line.y        = element_line(), #element_line()
          axis.ticks.x       = element_blank(), #element_blank()
  ...
  z2 <- data2$ranked.sig.pks.ms
  z3 <- data3$ranked.sig.pks.ds
  ...
  data_ll <- rbind(z1_df[-c(2)], z2_df[-c(2)], z3_df[-c(2)])
  data_ll$shade <- c(rep('WL', N), rep('MS', N), rep('DS', N))
  
  if (grepl('leaf', filename3a)) {
    ymax_value = 3750
    tick_increment = 500}
  if (grepl('pet', filename3a)) {
    ymax_value = 2000
    tick_increment = 500}
  
  ll <- ggscatter(data_ll, x = "wavenum", y = "value",
              color = "shade", palette = c("#E64B35FF", "#4DBBD5FF", "#7E6148FF"),
              shape = "shade",
              xlim = c(600, 1750),
              ylim = c(0, ymax_value),
              size = 5,
  ...
  if (i == 1) {
    ggsave("WT_leaf_1d_ll.jpeg", units="in", width = w, height = h, device='tiff', dpi=600)
  }
  
  if (i == 2) {
    ggsave("PhyA_leaf_1d_ll.jpeg", units="in", width = w, height = h, device='tiff', dpi=600)
  }
  
  if (i == 3) {
    ggsave("PhyB_leaf_1d_ll.jpeg", units="in", width = w, height = h, device='tiff', dpi=600)
  }
  
  if (i == 4) {
    ggsave("WT_petiole_1d_ll.jpeg", units="in", width = w, height = h, device='tiff', dpi=600)
  }
  
  if (i == 5) {
    ggsave("PhyA_petiole_1d_ll.jpeg", units="in", width = w, height = h, device='tiff', dpi=600)
  }
  
  if (i == 6) {
    ggsave("PhyB_petiole_1d_ll.jpeg", units="in", width = w, height = h, device='tiff', dpi=600)
  }
}
